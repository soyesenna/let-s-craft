...
(Scope note, corrected: this is the skill-contract half of the merge-block only — `enforcement.ts`'s existing `tool_call` block specifically cannot key off `land` time, since it only fires while `getActiveCraft()` matches this feature, and by `land` there is no active craft for it to key off (post-craft never calls `lsc_craft_init`, §1.5). That is NOT the same claim as "a code-level seam is structurally impossible here," though — `lsc_audit_begin`/`lsc_audit_validate` (B-1) already read and write this feature's persisted `test/.craft-state.json` directly, by exact root, with no active-craft singleton required (`src/craft/state.ts`'s `recordAuditCycleBegin`/`recordAuditValidated`), and `lsc_audit_validate`'s success additionally stamps a durable `auditValidated: {cycle, verdict, at}` marker onto that same persisted state. That marker is precisely the kind of thing a *new*, persisted-state-keyed enforcement seam (e.g. a `land`-time tool_call check, or a dedicated `lsc_land` tool, that refuses unless `auditValidated` names the current cycle/verdict) could check — proving such a seam is possible, not merely a prose-only limitation. Introducing that new seam is out of this feature's scope and deferred to a follow-up feature, not precluded by anything structural. `lsc_audit_validate` is the sole backstop for THIS feature, which is why calling it is not optional.)
