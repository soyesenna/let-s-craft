...
(Scope note: this is the skill-contract half of the merge-block only — no code-level `tool_call` block enforces it, because by the time `land` runs there is no active craft for `enforcement.ts`'s block to key off in the first place (post-craft never calls `lsc_craft_init`, §1.5); the enforcement seam that block relies on simply does not exist at this point in the pipeline. `lsc_audit_validate` is the sole backstop here, which is why calling it is not optional.)
