...
4. **Artifact paths (C8)** — mirror `src/artifacts/paths.ts` exactly, same replicate-by-hand discipline pre-craft/craft already establish (nothing in `src/main.ts` registers a tool for audit-directory/merge/worktree-removal setup, so this skill is the sole owner of replicating those conventions by hand):
   ...
   | Plan appendix (read) | `{worktreeAbs}/.lsc/crafts/{feature}/plan/appendix-{topic}.md` — deliberation detail moved out of `plan.md`'s core (full per-branch DR detail, pre-mortem in full, expanded test plan, external precedent, security/multi-repo detail); consult only when the audit needs the full reasoning behind a decision `plan.md`'s 1-line pointer references |
...
2. **Determine the audit cycle number `N`.** `glob .lsc/crafts/{feature}/audit/audit-*.md`, parse the numeric suffixes, `N = max + 1` (or `0` if the directory is empty/missing — C8). Then call `lsc_audit_begin(feature_dir)` — it records the current max `test/logs/run-N.log` index as this cycle's freshness threshold (B-1, cycle-freshness). §7.1's machine verdict validation depends on this having been called: without it, `lsc_audit_validate` still enforces exact-literal verdict parsing, but cannot confirm the eventual verdict is backed by a run from *after* this cycle began rather than a stale carried-over green log.
...
1. **Attempt `lsc_run_tests(feature_dir)` first.** As of B-1, this now succeeds even when this session does NOT hold `{feature}` as the active craft — the common case, since this skill deliberately never calls `lsc_craft_init` itself (§1.5). When no active craft matches, `lsc_run_tests` looks up the feature's persisted `test/.craft-state.json` (`findPersistedCraftState`, `src/craft/state.ts`) and, whenever it finds one with no unclosed open-release, runs `run_test.sh` and writes `test/logs/run-N.log` in the ordinary numbering sequence anyway — an **"audit-evidence" mode** that deliberately does NOT mutate any active-craft loop bookkeeping (`testsPassed`/`failureSignature`/`consecutiveFailures` are left untouched: there is usually no active craft to mutate in the first place, and even when a DIFFERENT feature's craft happens to be active this session, its state must never be contaminated by an unrelated feature's audit run). This is now the common **success** path, not a common failure — `lsc_run_tests` only refuses (`isError: true`) when there is no persisted craft state at all for `{feature}` (it truly never completed craft), or when an unclosed open-release blocks it (the same re-baseline guidance as before). This is exactly the mechanism `lsc_audit_begin`/`lsc_audit_validate` (§2.2, §7.1, B-1) depend on: without a fresh `run-N.log` reaching disk here, cycle-freshness could never be satisfied outside a same-session craft→post-craft run.
2. **On that error** (no persisted state at all, or an open-release block), **fall back to `bash` directly** — exactly `pre-craft`'s Stage 4 approach (`skills/pre-craft/SKILL.md` §6.4): run `bash {worktreeAbs}/.lsc/crafts/{feature}/test/run_test.sh` with the bash tool's own `cwd` parameter set to `{worktreeAbs}` (the implementation root from §2.3 — always the worktree now, except §2.3's own missing-worktree escape hatch, in which case use the project root instead) — the same `execCwd` selection `src/craft/run-tests.ts` uses internally (`craft.worktreeRoot ?? craft.projectRoot`), just invoked without the trusted-exec wrapper. Save the transcript into `audit/audit-{N}.md` yourself. **This transcript is NOT usable as `lsc_audit_validate`'s cycle-freshness evidence** — it does not produce a numbered `test/logs/run-N.log` (that numbering is owned exclusively by `lsc_run_tests`'s own `runFeatureTests`, in both its normal and its audit-evidence mode above) — so reaching this fallback is a sign `{feature}` never actually completed craft (no persisted state at all), not the routine path anymore. **This fallback is safe from the `tool_call` block by construction**: the block only fires when `getActiveCraft()` matches this feature (`src/craft/enforcement.ts`), and its bash check is a fail-closed substring match on the test dir path that doesn't distinguish read from write — but that's exactly the condition under which step 1 above already succeeded via `lsc_run_tests` instead (now true for both the active-craft AND the audit-evidence case). The two paths are mutually exclusive; there is no scenario where the bash fallback is both needed and blocked.
