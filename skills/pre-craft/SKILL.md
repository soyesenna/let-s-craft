...
**Review assignment implications.** Iteration 1's architect/critic review assignments must include the full core **and** every appendix in full. From iteration 2 onward, review assignments are core-plus-diff: point reviewers at the core plus only the appendix(es) that changed since the last iteration — do not re-hand the full appendix set every iteration once it has already been reviewed once.
...
**Progress visibility — one user-visible line at every stage/iteration boundary (mandatory, not optional polish).** At each of the following boundaries — architect review complete, critic review complete, a revision started, an AWC edit applied, `lsc-critic-recheck` passed — emit one line of plain user-visible text reporting it before moving to the next step (e.g. "architect 리뷰 완료 — critic 스폰 중", "AWC 적용 완료 — 재확인 스폰 중"). This applies identically when Stage 4 runs this same loop over `test/`. This is not cosmetic: a measured 13-hour run surfaced only 2 lines of visible progress text in total, and a 429 stall went unnoticed and unaddressed for 114 minutes as a direct result — visible progress at every boundary is what lets a stall be noticed and acted on instead of silently eating wall-clock time.
...
**Unresponsive-spawn non-approval rule.** If an architect or critic spawn (in this loop) ends in death, error, timeout, or a `**VERDICT:**` line that fails to parse — or its AWC diff-only re-check's `lsc-critic-recheck` spawn ends in death, error, timeout, or a `**RECHECK:**` line that fails to parse — that review is **inconclusive** — it never counts toward Full consensus, the AWC path, or any other passing outcome in point 4 below, no matter how the other side's review came out. Re-spawn it; if it keeps failing, stop the loop and report the repeated failure to the user rather than silently treating an inconclusive review as a pass.
