...
- 발급 트랜잭션(캡처된 lsc_confirm execute — CS6 (a)(b)(c)): **`RELEASE_CONSUMABLE_TAGS`가 정확히 `["[Canon Amendment]"]`(소비 tag allowlist exact)**; **(a) yes 발급 후 same-tag no/free answer/cancel 각각 슬롯 empty(3암)**; **발급 조건 = 태그 매칭 ∧ active craft ∧ yes 결합: active craft 있어도 무태그 yes는 무발급(durable 증거도 미기록)·무craft tagged yes는 무발급·`[Land]` yes는 `[Land]` capability 발급**; **(b) deferred confirm(resolve 지연 제어 fake ui) 대기 중 `clearActiveCraft`/`setActiveCraft` 후 응답이 yes여도 무발급(2암)**, **대기 중 `recordTestResult`로 동일 identity active object가 spread 교체돼도 yes는 정상 발급(참조 동일성이 아니라 `sameCraftIdentity` 실사용 판별)**; **(c) `.craft-state.json` write 실패 유도 시 durable 증거·슬롯 모두 미설치(선설치한 기존 승인도 prompt 진입 선철회로 잔존 없음)(1암)**.
- 증거: recordReleaseApproval/recordOpenRelease persist 라운드트립(파일 재독 필드 단언) + persist 실패 시 in-memory 미반영(CS3) — **recordReleaseApproval 실패뿐 아니라 recordOpenRelease persist 실패 시에도 openRelease 미기록·`getActiveCraft().openRelease` undefined·기존 approval의 consumedAt 미스탬프(fail-closed)**, consumedAt 스탬프, fingerprint 존재/부재(undefined) 양쪽, **nonce는 UUID v4 형태·issuedAt는 ISO 8601 왕복(형식 계약)**, **releaseApproval 단일 슬롯 덮어쓰기 — 연속 발급 2회 후 마지막 증거만 잔존, merge가 아닌 replacement로 이전 consumedAt 미잔존(F-14)**.
...
**W2 — integration** (`assets/test/craft-release-flow.test.ts` 신규 — **actual registered tool execution (CS7)**: mock `pi`(선례 test/ask-schema.test.ts:1-42)로 `registerAskTools`(ask.ts:541)/`registerHashManifestTools`(hash-manifest.ts:385-388)/`registerCraftReleaseTool`(release.ts:43)/`registerRunTestsTool`(run-tests.ts:186)의 definitions를 capture하고 **등록된 `execute`를 직접 호출**한다; release 소비도 `callTool(tools, "lsc_craft_release", { reason }, ctx)`로 **등록된 `lsc_craft_release` execute를 default Vitest 층에서 실제 호출**한다(직접 `performCraftRelease` 호출 아님 — reason 전달·core 위임 배선까지 기계 검증, CS1). 실제 tmpdir 프로젝트 루트; helper 조립 시뮬레이션은 production wiring 검증으로 불인정. **AC4 E2E arm의 '재량' 여부와 무관하게 이 integration은 필수다**):
...
4. close 후 **actual `lsc_verify_hash` execute 복귀**(성공); 이어 **`clearActiveCraft()` 후 inactive 상태에서도 verify 복귀(성공)·run_tests는 legacy exact 문자열("no active craft … Call lsc_craft_init first.") 반환** — closed evidence를 open으로 오판하지 않음(B6 inactive+closed). 별도 legacy-only arm: 구버전 state 직서 후 registered `lsc_run_tests`가 동일 legacy 문자열 반환.
5. **두 번째 actual init** 후에도 `releaseApproval`(consumedAt 포함)·closed `openRelease` 증거 보존 (CS4 retention).
6. **manifest atomic-save는 완료되되 craft-state atomic write만 실패**하도록 `Date.now` 고정 + state writer의 `{basename}.{pid}.{Date.now()}.tmp` 경로에 디렉터리 선점(atomic-write.ts)으로 유도 — **actual init execute 실패 + manifest 단계 완료(재독 상이) + active 미공개(getActiveCraft() undefined) + persisted open 잔존(closedAt 미설정)으로 verify 계속 거부** (CS5 fail-closed; chmod-전체-디렉터리 probe가 못 미치는 persist-before-publish commit point 도달).
