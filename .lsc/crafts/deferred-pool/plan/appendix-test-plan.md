...
- **실패 경로(AC3)**: merge conflict fixture→자동 abort 후 **사후 상태 검증(branch/OID/clean) 통과=merge-aborted** + isError 3요소(원복·nonce 소비·재승인) / **abort 실패 유도(사후 상태 검증 불통과)→merge-recovery-failed(원복 미보장 명시)** / **target-dirty: tracked 변경 fixture→거부, untracked-only fixture→통과(양성)** / **source ref drift(승인 후 source branch 이동)→approved sourceOID가 merge됨 단언** / dirty→merged-but-not-cleaned(worktree 보존 단언)→mode=force-clean-only 재승인→force 성공 / **prune 실패 유도→cleaned-but-not-pruned(실제 state flags 단언)** / force 승인 없이 force 호출→scope-mismatch / locked·메인 리포·미등록(membership) 각 거부 / 삭제 직전 재검사 배선(재검사 시점 관찰 실패 유도로 확인).
...
- 본체: `npm test`(vitest run 전수 — AC12) + `tsc`. AC별 파일은 위 신규/확장 6파일(craft-land·artifacts-worktree·artifacts-scaffold·research-ledger·doctor + craft-state/skill-contract 확장)이 담당.
