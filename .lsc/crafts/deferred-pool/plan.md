...
| 규모 | 6 Step / 9 커밋 — 신규 src 5(land.ts, scaffold.ts, doctor.ts, research/ledger.ts, research/claims-tool.ts) + 개조 src 7(destructive-approval.ts, ask.ts, state.ts, verdict.ts, worktree.ts, gitignore.ts, main.ts) + paths.ts 헬퍼 1 + 스킬 2(pre-craft, post-craft SKILL.md) + 테스트 ~9 |
| 복잡도 | HIGH (컴포넌트 7종, 보안 게이트 1, 신규 진단 표면 1) |
| 작성일 | 2026-07-19 |
| 개정 | iteration 2 — architect blocking-redesign + critic REVISE 전수 반영(원장: plan/plan-1.md). **iteration 2 리뷰 마감**: architect AWC-equivalent Change Spec 11항 verbatim + critic APPROVE-WITH-CHANGE 보정 2건(C1·C2)·minor 3건 적용(원장: plan/plan-2.md). **Stage 4 test-canon 루프 iteration 1**: architect BLOCKING_REVISE rec 2·6 + critic REVISE C3·C4·M5 반영 — LandEffects 주입 seam(3b)·claims-tool.ts thin wrapper·atomic-write 주입 port 3건 소폭 개정(원장: plan/plan-3.md) |
...
| F | claims.json 스키마 + transition-aware 평가기(tombstone 부활 금지) + lsc_claims 도구(decision 필드 단독 소유자) + 정본-projection 계약 산문 | src/research/ledger.ts(신규 — fs 무접촉 순수), src/research/claims-tool.ts(신규 — SDK-independent thin wrapper), src/artifacts/paths.ts(craftClaimsPath), src/main.ts, skills/pre-craft/SKILL.md |
...
- **LandEffects 주입 seam (C3/rec2)**: Phase X의 effect 호출 전체를 lsc_land 오케스트레이터가 주입받는 좁은 effect port `LandEffects{merge(sourceOID), abortMerge(), verifyPostRecovery(expected), inspectRemovalTarget(candidate), remove(path), prune()}`로 배선 — 기본 구현은 실 git(순수 게이트 리듀서 무변경, 제약 9). happy·실 conflict fixture는 실 git으로 유지하되, 외부에서 결정론적으로 만들 수 없는 희귀 실패만 이 port에 결정론 주입: verifyPostRecovery 실패→merge-recovery-failed, prune 실패→cleaned-but-not-pruned, 삭제 직전 inspectRemovalTarget 재검사 실패→validation-reject — 동일 production 오케스트레이터 아래 상태+effect 불변식으로 단언. `inspectRemovalTarget`은 이 port 경유로 **삭제 직전 실호출**됨을 계약으로 명시(구현만 하고 미배선 시 dead-code false-green 차단) — 재검사 관찰 실패 주입 시 removeWorktree 미호출 단언. happy path는 2-parent merge commit(--no-ff 변별)+seeded stale metadata prune 제거로 강화.
...
- **AC1·AC2·AC3**: 사유 코드 전 변형 + production 전 경로(발급→소비, 재승인 왕복 포함) + 실패 경로(conflict abort 사후검증·abort 실패→merge-recovery-failed·target-dirty(tracked)/untracked-only 양성·source ref drift에도 approved sourceOID merge·prune 실패→cleaned-but-not-pruned·삭제 직전 재검사 실패→validation-reject·dirty→force·locked/main/미등록) — 희귀 실패(merge-recovery-failed·cleaned-but-not-pruned·validation-reject)는 LandEffects port 결정론 주입, appendix-test-plan W2-a.
...
- `lsc_claims` 도구 — **신규 src/research/claims-tool.ts(SDK-independent thin wrapper)에 `registerClaimsTool` execute 소재**(C4/rec6): ledger.ts는 fs 무접촉 순수 유지, fs 접촉(읽기·경로 해석·쓰기)은 전부 wrapper에, main.ts는 wrapper 경유로만 등록(pure boundary 불변식 — pure module에 tool execute 금지). 입력 `{feature_dir}` — **kebab-case slug만 허용**(`.`/`..`/NUL/경로 구분자 주입은 write 전 거부). root는 **`worktreePath(ctx.cwd, feature)`로 고정**(all-in-worktree 정책 — main cwd=projectRoot에서 잘못된 project-root artifact 읽기/쓰기 차단) + registered worktree 확인 + claims parent realpath containment + target/parent symlink 거부. **전 claims를 먼저 parse+validate+evaluate — 한 개라도 invalid면 byte 0 변경**, 전부 성공한 뒤 `craftClaimsPath(worktreeRoot, feature)`에 **주입 가능 atomic-write port로 정확히 1회 쓰기**(M5/rec6 — writer port 기본값 `writeFileAtomicSync`, 테스트는 이 port를 주입해 call count·path·payload로 exactly-once 검증; 별도 decisions.json·TTL 도입 금지). decision write-back은 per-claim 현재 `decision` 교체 + projection 요약 반환. **decision 필드는 도구 단독 소유**(사람/LLM은 claims/evidence만 기록 — schema+skill 규약으로 강제). 스키마 위반 isError.
...
- **AC11**: 사다리 매트릭스(ledger.ts 순수) + tombstone 부활 금지(transition) + claims-tool wrapper registrar 캡처 + **주입 atomic-write port exactly-once(call count·path·payload)** + **wrong-root/traversal/symlink(target·parent)/invalid-one-no-write 거부** + skill-contract 문구.
...
| 3 | 실패 경로: merge conflict→자동 abort+사후 상태 검증(merge-aborted/merge-recovery-failed 구분) / target-dirty(tracked) 거부·untracked-only 통과 / dirty→merged-but-not-cleaned→force-clean-only 재승인 성공(mode scope 결박) / prune 실패→cleaned-but-not-pruned / 삭제 직전 재검사 실패→validation-reject(inspectRemovalTarget port 실호출·removeWorktree 미호출) / locked·main·미등록 불가 — 희귀 실패 3종은 LandEffects port 결정론 주입 | 3b | integration(LandEffects port 주입) | test/craft-land.test.ts |
...
| 11 | 평가기 사다리(ledger.ts 순수) + tombstone 부활 금지(transition 입력) + lsc_claims wrapper(claims-tool.ts) registrar 캡처 + atomic-write port exactly-once + 정본-projection 문구 | 5a | unit+registrar 캡처+skill-contract | test/research-ledger.test.ts·test/claims-tool.test.ts(신규) |
...
- Architect: AWC-equivalent (iteration 2) — Change Spec 11항 verbatim 적용 완료(redesign 불필요 판정). **Stage 4 test-canon 루프 iteration 1**: BLOCKING_REVISE — rec 2(LandEffects port)·rec 6(claims wrapper+atomic-write port) 반영, rec 1·3·4·5·7은 apply-only 정본 수정(test 작성자 소관).
- Critic: APPROVE-WITH-CHANGE (iteration 2) — MAJOR 2건 보정(C1 신뢰 경계 타깃·C2 target-dirty tracked 한정) + minor 3건 적용. **Stage 4 test-canon 루프 iteration 1**: REVISE — 재결정 2건 C3(land effect port)·C4(claims wrapper)+atomic-write port 반영, C1·C2·C5·m1 apply-only 정본 수정 후 single-diff recheck 대기.
- 이터레이션 원장: `plan/plan-1.md` (iteration 1) · `plan/plan-2.md` (iteration 2 — 두 리뷰 판정·적용 diff 요약) · `plan/plan-3.md` (Stage 4 test-canon 루프 iteration 1 — architect BLOCKING_REVISE·critic REVISE 판정 + 소폭 개정 3건 diff)
