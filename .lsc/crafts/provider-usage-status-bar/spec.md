...
## Goal
Add a **persistent status bar rendered directly below the omp prompt input** (via the plugin's `ctx.ui.setWidget(..., {placement:'belowEditor'})`, NOT the existing top status border) that displays, for **every logged-in provider account** (including multiple accounts of the same provider), each account's usage across **all usage windows omp reports** (5-hour, 7-day, and any model-scoped-weekly / monthly / extra-credit windows). It is **display-only** (no interaction), **always shown** when accounts exist, and **auto-condenses** when the terminal is too small.
...
5. **Non-destructive rendering.** The bar MUST NOT corrupt or shift the prompt input; on session change it MUST re-render (omp clears hook widgets on new/switch session).
...
- No interactivity (no clicking/expanding/switching accounts from the bar) in this iteration.
...
3. **Per-window format:** For each usage window of a subscription account, the row renders `<window-label> <mini-bar ▓▓▓░░> <used%> <reset-countdown>` (e.g. `5h ▓▓▓░░ 33% 2h10m`), for every window omp returns (5h, 7d, model-weekly, monthly, extra).
...
7. **Degradation:** When terminal width/height is insufficient, the bar auto-condenses, prioritizing windows **nearest their limit**; the prompt input is never disrupted.
...
Testability note (feeds Stage 4): criteria 2–5 and 9 are unit-testable on the pure data→display transform (map `UsageReport[]` → grouped rows → formatted strings) with fixture `UsageReport` inputs; criteria 1, 6, 7 (live TUI placement/refresh/clipping) are integration/manual — the P4 clipping check is the deferred craft-time live probe.
