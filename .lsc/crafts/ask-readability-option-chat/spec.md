...
1. 질문 제목/옵션 label/설명/푸터/채팅 패널이 lets-craft 팔레트의 위계(굵기·색·여백·구분선)로 렌더되고, 렌더 출력이 결정론적 불변식 테스트로 고정된다 — 9역할 매핑·역할 간 pairwise 구분·styled run의 RESET 종결. (exact RGB/ANSI byte는 비계약 — 팔레트는 craft-tunable이므로 값 자체가 아닌 불변식이 계약이다. D4 재비준으로 '스냅샷 고정' 문구를 대체.)
2. 호스트 Theme의 라이트/다크 모드 각각에서 팔레트가 적응해 대비가 유지된다 — 모드별 불변식 테스트(light≠dark 적응 + 명시 reference background 대비 방향).
