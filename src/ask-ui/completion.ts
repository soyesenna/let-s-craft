...
import type { AssistantMessage, Context, StreamOptions } from "@oh-my-pi/pi-ai";
...
/**
 * Build the live CompletionPort bound to the invocation context. Each `run` drives pi-ai `stream` with
 * the frozen request: `{ systemPrompt: [...] }`, the converted messages, NO tools (no-tools side call),
 * and the apiKey RESOLVER forwarded by identity (so the SDK can re-invoke it for rotation). A provider
